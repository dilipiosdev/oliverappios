...
class FinalRegistroDeVintasViewController: UIViewController, UITableViewDelegate,UITableViewDataSource,UITextFieldDelegate {
    ...
    func updateDB() -> Void {
        ...
        userRef.child("My Companies/\(companyChildValue)").child("Product Bill").observeSingleEvent(of: .value) { (snapshot) in
            if let array = snapshot.children.allObjects as? [DataSnapshot]
            {
                for i in 0 ..< array.count
                {
                    ...
                    Database.database().reference().child("My Companies").child(self.companyChildValue).child("My Products").child(obj.key).child("Buyers").child("People").child(self.bill_Id).updateChildValues([
                    ...
                    Database.database().reference().child("My Companies").child(self.companyChildValue).child("My Products").child(obj.key).observeSingleEvent(of: .value, with: { (snapshot) in
                        ...
                        if let districs = snapshot.value as? Dictionary<String,Any>
                        {
                            ...
                            let stockValue = Double(obj.childSnapshot(forPath: "quantity").value as? String ?? "0.00") ?? 0.00
                            let value = mainStock  - stockValue
                            let valueStr = value
                            Database.database().reference().child("My Companies").child(self.companyChildValue).child("My Products").child(obj.key).updateChildValues([
        ...
        userRef.child("My Companies/\(companyChildValue)").child("Product Bill").observeSingleEvent(of: .value, with: { (snapshot) in
    ...
    func updateMyBillMethod() -> Void {
        ...
        if(matchType == "Final")
        {
            let dict:[AnyHashable : Any] = [
                    ...
                    "customer_academic_degree": txtEducatvo.text ?? "",
            ...
            Database.database().reference().child("My Companies").child(companyChildValue).child("Customers")
                .child(customerIDStr).updateChildValues(dict)
        }
       
        
        self.dismiss(animated: true, completion: nil)
//        let storyBoard : UIStoryboard = UIStoryboard(name: "Comercial", bundle:nil)
//        let nextViewController = storyBoard.instantiateViewController(withIdentifier: "FinalizerSuccessViewController") as! FinalizerSuccessViewController

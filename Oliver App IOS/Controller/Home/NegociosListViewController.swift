...
class NegociosListViewController: UIViewController,UITableViewDelegate,UITableViewDataSource {
    ...
    override func viewDidLoad() {
        ...
        self.subViewButton.layer.addSublayer(GlobleClass.backGrayGroundColor())
        ...
        userRef.observeSingleEvent(of: .value) { (snapshot) in
            if let companyArray = snapshot.children.allObjects as? Array<Any>
            {
                for item  in companyArray {
                    ...
                    let uid = dict.childSnapshot(forPath: "uid").value as? String ?? ""
                    if(uid == appDelegate.userID)
                    {
    ...
    @objc func gestionarMethod(_ sender:UIButton) -> Void {
        ...
        if(indexPath != nil)
        {
            ...
            nextViewController.companyDetailDict = self.comapnyListArray[indexPath!.row] as! DataSnapshot

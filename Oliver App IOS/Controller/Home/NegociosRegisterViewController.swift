...
class NegociosRegisterViewController: UIViewController, UIScrollViewDelegate,UINavigationControllerDelegate,UIImagePickerControllerDelegate,UITableViewDelegate,UITableViewDataSource {
    ...
    func getImagefromFireBase() -> Void
    {
       ...
       // lblRegister2.textColor = .gray
        let userRef = Database.database().reference().child("Users/\(appDelegate.userID)").child("Company Registration")
    ...
    func getDataFromFireBase() -> Void {
        ...
        hud.textLabel.text = "Loading"
        hud.show(in: self.view)
        let userRef1 = Database.database().reference().child("Users/\(appDelegate.userID)").child("Company Registration")
    ...
    func uploadImageCompany(image: UIImage,Key:String, StorageKey: String, match: String) -> Void {
        ...
        if(data.count > 0)
        {
            let storge = Storage.storage().reference().child(Key).child(appDelegate.userID)
    ...
    @IBAction func buttonContinue5(_ sender: Any) {
        ...
        hud.show(in: self.view)
          ...
          if (response.response != nil)
          {
             ...
             {
                 let dateStrLocal = self.lblYear.text! + "-" + self.lblYear.text! + self.lblMonth.text!.uppercased() + "-" + self.lblDate.text!
...
//             if let urlText = url?.absoluteString
//             {
//                Database.database().reference().child("My Companies").child("Users/\(appDelegate.userID)\(timestamp)").child("Company Registration").updateChildValues(["qr_code_image": urlText])
//               }
//            })
//         }
//       }
//      }
        let timeStmpStr = String(format: "%.f", timestamp)
        Database.database().reference().child("My Companies").child("\(appDelegate.userID)\(timeStmpStr)").updateChildValues([
